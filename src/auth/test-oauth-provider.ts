...
export class TestOAuthProvider implements OAuthClientProvider {
  ...
  /**
   * Prepare token request for client_credentials grant type
   */
  prepareTokenRequest(scope?: string): URLSearchParams | undefined {
    const grantTypes = this.config.clientMetadata.grant_types;

    // Check if we're using client_credentials grant
    ...
    // For authorization_code grant, return undefined to use default behavior
    return undefined;
  }
}
